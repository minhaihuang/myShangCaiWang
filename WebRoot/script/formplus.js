...
var DatePickers = new Class({
  ...
  caption : function() {
    ...
    if (a.prev.year) {
      e.clone().addEvent("click", function() {
        this.navigate("y", -1)
      }.bind(this)).inject(c)
    }
    new Element("span").set("text", this.curFullYear).addEvent("mousewheel", function(g) {
      g.stop();
      this.navigate("y", (g.wheel < 0 ? -1 : 1));
      this.build()
    }.bind(this)).inject(c);
    if (a.next.year) {
      d.clone().addEvent("click", function() {
        this.navigate("y", 1)
      }.bind(this)).inject(c)
    }
    ...
    new Element("span").set("text", this.options.months[this.curMonth]).addEvent("mousewheel", function(g) {
      g.stop();
  ...
  tbody : function() {
    ...
    g.addEvent("mousewheel", function(d) {
      d.stop();
...
var validator = new Abstract({
  test : function(b, f) {
    ...
    var l = [], h = f.get("required"), e = f.get("vtype"), k = f.get("caution");
    extra = b.get("extra");
    if (e && e.contains("&&")) {
      e = e.split("&&")
    }
  ...
  bindBlurWithValidator : function(c, a) {
    ...
    b.removeEvents("blur").addEvent("blur", function() {
...
if ($("goodsselect")) {
  $("goodsselect").addEvent("click", function(b) {
    b = new Event(b);
...
if ($("pkgselect")) {
  $("pkgselect").addEvent("click", function(b) {
    b = new Event(b);
...
if ($("giftselect")) {
  $("giftselect").addEvent("click", function(b) {
    b = new Event(b);
